...
const IO = React.createClass({
    render: function () {
        var sortTarget = '',
            showSortTarget = (this.props.sortedId !== this.props.data.parentId) && this.props.showSortTarget ? true : false;

        if (showSortTarget) {
          sortTarget = <div onClick={()=> {this.props.sortTargetClicked(this.props.data.id, this.props.data.parentId); } } className='sort-target io__sort-target'></div>;
        }

        return (
            <div data-id={this.props.data.id} data-parent-id={this.props.data.parentId} className={'io io--' + this.props.data.type + ' ' + this.props.toggleClass}>
                <header>
                    <i className='sortable-handle io__sortable-handle' onClick={()=>this.props.sortHandleClicked(this.props.data.id, this.props.data.parentId)}></i>
